...
- SVGアイコンを `Icon.svelte` コンポーネントに抽出（約15種類を管理し、19箇所のベタ書きを置換）
- 設定画面を左ペイン（大分類）+ 右詳細の2カラムレイアウトに再設計し、狭い幅ではカテゴリボタンを横並び・均一幅で表示
- `CommandSettingsModal` を新しい `SettingsModal` に統合（既存の Appearance / AI Tool / Expanded Commands を General / AI Tool / Commands の3セクションに再編）
...
- `command-preferences`: 設定画面のレイアウトを左ペイン+右詳細の2カラム構成に変更し、General / AI Tool / Commands のカテゴリ切り替えを追加
